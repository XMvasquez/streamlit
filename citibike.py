...
sidebar = st.sidebar
st.sidebar.title('Anahi Ximena Sanchez Vasquez ')
st.sidebar.text('zs2006727@estudiantes.uv.mx')
...
hour_to_filter = sidebar.slider('hour', 0, 23, 23)
filtered_data = data[data[DATE_COLUMN].dt.hour == hour_to_filter]

st.text('Mapa de recorridos iniciados a las %s:00' % hour_to_filter)
st.map(filtered_data)

st.sidebar.image("Foto credencial.jpg")
